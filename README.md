...
> - `fig1_atlas.py`, `supp_fig_s7_bhlhe_coexpr.py` — require SEA-AD h5ad files (~3.3–36 GB). Download with `bash code/phase1_QC/01_data_acquisition.sh` (requires AWS CLI).
> - `supp_fig_s3_qc.py` — requires `results/phase1/trajectory/microglia_trajectory.h5ad`, generated by running `sbatch code/slurm/27_trajectory_paga.slurm` (128 GB RAM, ~8 h on TRUBA `barbun` partition) after downloading the SEA-AD microglia h5ad.
