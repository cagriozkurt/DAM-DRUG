...
> **STATUS 2026-09-11 — job 6346282 COMPLETED but result FLAGGED UNRELIABLE; dual-window DB rebuild RUNNING (TRUBA).** `s1_02` (wide 10kbp-up/10kbp-down cisTarget DB) COMPLETED. `s1_03` job **6346282** ran `ctx`+`aucell` clean (3 env-drift bugs fixed en route: bare `pyscenic` off PATH, `pkg_resources`/setuptools>=81, `np.object` removal, `libstdc++` CXXABI via LD_PRELOAD, pandas `Series.iteritems()` removed in pandas>=2.0 — all via a `sitecustomize.py` shim on `PYTHONPATH`) but the RESULT is suspect: only 9/many regulons retained and **0/11 target TFs survived, including IKZF1 itself**, despite every target TF having a present JASPAR2026 motif and a large GRNBoost2 candidate-target set (IKZF1: 5967 targets, well above the 455-target median). Diagnosed as enrichment dilution from scoring against a single wide window alone (verified the rankings feather itself is structurally sound — IKZF1's own motif correctly ranks the IKZF1 gene at 687/27090) — aertslab's own production databases are scored as a matched wide+narrow pair. Rebuilding: narrow 500bp-up/100bp-down window build submitted as job **6347274** (TRUBA, queued), `s1_03` updated to score both windows together via `pyscenic ctx`'s multi-db support. `s1_04` (benchmark/nulls) not yet submitted — needs the dual-window `s1_03` rerun first.
