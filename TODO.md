...
* [~] **Production T-REMD, in progress — moved entirely to local GPU.** TRUBA `akya-cuda` stayed fully allocated with no ETA (`6346303_[0-2]` queued PD/Priority for hours) → moved to the user's local RTX 3060 Mobile (6GB VRAM), 2026-09-11. Re-validated the smoke test there first (same result: 49 exchanges, clean, GPU peaked 1.3GB of 6GB free) before committing. Found along the way: `s4_02` equilibration had ONLY ever been run for GLUE0231 (`array=0`) across the whole debugging session — GLUE0246/GLUE0692 had no `npt.gro`/`npt.cpt` at all; replicated `s4_02`'s steps locally for both. All 3 complexes' 100ns×8-replica T-REMD production ran CONCURRENTLY on debian-local (24 replicas total, GPU ~3.8GB/6GB).
  ...
  **2026-09-13 outage + recovery.** All three runs stalled simultaneously at ~07:09 (GLUE0246 hard-crashed with a CUDA/GPU error; GLUE0231/GLUE0692 hung alive-but-frozen) — root cause: NVIDIA persistence mode is Disabled on this laptop (no sudo to fix) and the dGPU's runtime power management let CUDA lose the device between calls (`nvidia-smi`/NVML still saw it; `cudaGetDeviceCount` didn't). User rebooted, restoring GPU function. During recovery a self-inflicted issue surfaced: an isolated single-rep GPU-sanity-check run right after reboot used `-cpi` and silently advanced *only* rep0's checkpoint by 100 steps (GROMACS `-cpi` always overwrites the input checkpoint regardless of `-noappend`), permanently desyncing rep0 from rep1–7's replica-exchange step counter and hanging the whole ensemble at the next exchange barrier. Fixed by topping up rep1–7 by the same 100 steps (CPU-only, negligible) to re-align all 8 at step 8,951,200, and relaunching with `-noappend` throughout to keep each checkpoint's file-tracking metadata self-consistent (GLUE0246/GLUE0692, whose own append-mode restart also silently hung, needed the same `-noappend` treatment). All 3 confirmed genuinely advancing post-recovery (not just alive): GLUE0231 +30ps, GLUE0246 +14ps, GLUE0692 +15ps over a 3-minute window, GPU 100% util, zero data loss (checkpoints were all within minutes of the original stall).
  ...
  All corresponding TRUBA jobs (`6346303_[0-2]`, `6347306`) cancelled — everything for this section runs on the local GPU, none on TRUBA. **Caveat for the user:** persistence mode being off means this GPU power-loss failure mode can recur; `sudo nvidia-smi -pm 1` would likely prevent it.
