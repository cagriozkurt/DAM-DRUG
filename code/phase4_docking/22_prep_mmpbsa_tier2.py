...
def prep_complex(target: str, chembl_id: str, vina_score: float) -> bool:
    ...
    if not ligand_sdf.exists():
        ...
        result = subprocess.run(
            [OBABEL, "-ismi", str(smi_file),
             "-O", str(ligand_sdf), "--gen3d", "--minimize", "--ff", "MMFF94s", "-p", "7.4"],
            capture_output=True, text=True
        )
        if not ligand_sdf.exists():
    ...
    # 4. Copy receptor PDB (strip HETATM — pdb2gmx handles standard residues only)
    ...
    if not rec_src.exists():
        ...
        return False
    rec_lines = [l for l in rec_src.read_text().splitlines(keepends=True)
                 if not l.startswith("HETATM")]
    n_removed = rec_src.read_text().count("\nHETATM")
    if n_removed:
        log.info(f"  Stripped {n_removed} HETATM records from receptor (zinc, ligands)")
    (work_dir / "receptor.pdb").write_text("".join(rec_lines))
