...
with h5py.File(COUNTS_H5, "r") as f:
    ...
    if len(indptr) == n_genes + 1:
        ...
        raise ValueError(f"Cannot infer shape: indptr len={len(indptr)}, n_cells={n_cells}, n_genes={n_genes}")
...
# ── Plot: dot plot ─────────────────────────────────────────────────────────────
# Focus on SLIT2 and ROBO2 (the key pair)
genes_plot = [g for g in ["SLIT2", "ROBO2", "GAS6", "MERTK"] if g in target_idx]

# Order cell types: interneurons first (SLIT2 senders), microglia last (ROBO2 receiver)
...
# Normalise mean_expr for dot size/color
stats_pivot_mean = stats.pivot(index="cell_type", columns="gene", values="mean_expr").reindex(CT_ORDER)
stats_pivot_pct  = stats.pivot(index="cell_type", columns="gene", values="pct_expr").reindex(CT_ORDER)
...
for ax, gene in zip(axes, genes_plot):
    ...
    # Normalise mean for color scale
